<template>
  <div v-if="showPopup2" class="popup-overlay" @click="closePopup2">
    <div
      class="popup-content"
      @click.stop
      :class="{ 'popup-disabled': loading }"
    >
      <button class="close-button" @click="closePopup2" :disabled="loading">
        &times;
      </button>
      <div class="popup-body">
        <div class="popup-text">
          ...
          <p class="pop-desc">
            احصل على عام كامل من الفوائد والمزايا بنفس تكلفة نصف العام! 🥳
          </p>
          <form
          ...
          >
            ...
            <button
              ...
              id="adults-pop-ar"
            >
              التسجيل في عروض الجمعة البيضاء
            </button>
          </form>
          ...
          <p class="pop-bottom">
            تُحفظ التفاصيل الشخصية وتستخدم فقط فيما يتعلق بطلبك فقط. يُرجى قراءة
            <a href="/ar/terms-and-conditions">شروط الاستخدام</a> لمزيد من
            المعلومات.
          </p>
        </div>
...
export default {
  ...
  methods: {
    ...
    register() {
      ...
      axios
        .post(`/api/free-session-forms`, payload, { headers })
        .then((response) => {
          ...
          if (response.status === 201) {
            ...
            window.location.href = "https://fridayoffer.monglish.co.uk/";
          } else {
            console.error(
              "Error in response:",
