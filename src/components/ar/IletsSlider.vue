...
export default {
  ...
  setup() {
    ...
    const autoplay = () => {
      currentSlide.value = (currentSlide.value + 1) % clubs.value.length; // Loop back to the first slide
    };
    ...
    onMounted(() => {
      ...
      autoplayInterval = setInterval(autoplay, 3000); // Set interval to change slide every 3 seconds
    });

    onBeforeUnmount(() => {
      window.removeEventListener('resize', updateItemsToShow);
      clearInterval(autoplayInterval); // Clear the interval on component unmount
    });
